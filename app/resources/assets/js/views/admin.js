...
; $(function () {
						...
						$.ajax({
							...
							success: function (response) {
								if (!response.successful) {
									$('#import-student-result').html('<p style="color: red"><b>An error has occurred.</b><br>' + response.message + '</p>');
									return;
								}
	...
	*/
	...
	$(".js-delete-pe").on('click', function (e) {
		...
		let content = 'Are you sure you want to delete this evaluation?';
		let type = 'red';
		let btnClass = 'btn-danger';

		if ($(e.target).data('show-warning') == false) {
			content += '<br>No questions have been answered so this is a safe action.'
			type = 'info';
			btnClass = 'btn-info';
		}
