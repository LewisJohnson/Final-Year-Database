	...
	@foreach($transactions as $transaction)
		<tr>
			...
			@if(Session::get("db_type") == "ug")
				@if($project = SussexProjects\ProjectUg::where('id', $transaction->project_id)->first())
					<td><a href="{{ action("ProjectController@show", $project->id)}}">{{ $project->title }}</a></td>
				@else
					<td></td>
				@endif
			@else
				@if($project = SussexProjects\ProjectMasters::where('id', $transaction->project_id)->first())
					<td><a href="{{ action("ProjectController@show", $project->id)}}">{{ $project->title }}</a></td>
				@else
					<td></td>
				@endif
			@endif
			@if($student = SussexProjects\User::where('id', $transaction->student_id)->first())
				<td>{{ $student->getFullName() }}</td>
			@else
				<td></td>
			@endif
			
			@if($supervisor = SussexProjects\Supervisor::where('id', $transaction->supervisor_id)->first())
				<td>{{ $supervisor->user->getFullName() }}</td>
			@else
				<td></td>
			@endif

			@if($admin = SussexProjects\User::where('id', $transaction->admin_id)->first())
